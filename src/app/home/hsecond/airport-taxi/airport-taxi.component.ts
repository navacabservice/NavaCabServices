...
export class AirportTaxiComponent {
  pickup = '';
  roundTripOption = 'From Airport'; // "To Airport" or "From Airport"
  date = '';
  time = '';
  phone = '';
  name = '';
  ...
  submitForm() {
    ...
    if (this.isRoundTrip && this.roundTripOption) {
      message += `*Round Trip Option:* ${this.roundTripOption}\n`;
    }
  ...
  if(!this.isRoundTrip){
    ...
    if (
      ...
      (this.isRoundTrip && this.roundTripOption === 'From Airport' && this.Flight)
    ) {
  ...
  resetForm() {
    ...
    this.roundTripOption = 'From Airport';
  }
}
